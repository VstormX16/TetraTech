...
- `api` image'i artik tum gerekli backend dosyalarini kopyalar.
- `neural_decision_engine.py` ve `tetra_neural_weights.npz` deploy icinde eksik kalmaz.
- HERMES klasoru dinamik bulunur; Turkce klasor adina bagli kirilgan `COPY` yapisi kaldirildi.
- `sim` servisi de repository icinden `server.py` dosyasini dinamik olarak bulur.
- Hem `api` hem `sim` hem de `frontend` icin healthcheck tanimlidir.
