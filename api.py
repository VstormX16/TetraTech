...
# HERMES AI module path

def resolve_hermes_dir():
    base_dir = os.path.dirname(__file__)
    candidates = []

    env_name = os.getenv("HERMES_DIR_NAME")
    if env_name:
        candidates.append(os.path.join(base_dir, env_name))

    for entry in os.listdir(base_dir):
        entry_path = os.path.join(base_dir, entry)
        if os.path.isdir(os.path.join(entry_path, "hermes_db")):
            candidates.append(entry_path)

    for candidate in candidates:
        if os.path.isdir(os.path.join(candidate, "hermes_db")):
            return candidate

    raise FileNotFoundError("HERMES directory not found. hermes_db is missing.")


HERMES_DIR = resolve_hermes_dir()
